...
class peephole
{
    private:
        ...
        bool mcword(size_t idx, std::string name)
        {
            idx += current;
            return ( idx < code.size() &&
                     code[idx].type == codew::word &&
                     code[idx].value == name );
        }
        bool mlabel(size_t idx)
        {
            idx += current;
            return idx < code.size() && code[idx].type == codew::label;
        }
        bool mlblw(size_t idx)
        {
            idx += current;
            return idx < code.size() && code[idx].type == codew::word &&
                    code[idx].value.find("lbl") != std::string::npos;
        }
        bool mword(size_t idx)
        {
            idx += current;
            if ( idx < code.size() && code[idx].type == codew::word )
            {
        ...
        {
            idx += current;
            if ( idx < code.size() && code[idx].type == codew::byte )
            {
                auto s = code[idx].value;
                return ( s.find_first_not_of("0123456789") == s.npos );
            }
        ...
        {
            idx += current;
            if ( idx < code.size() && code[idx].type == codew::label )
        ...
        int16_t val(size_t idx)
        {
            idx += current;
            if ( idx < code.size() &&
                 ( code[idx].type == codew::word || code[idx].type == codew::byte ) )
